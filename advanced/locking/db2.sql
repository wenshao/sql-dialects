...
--
-- 参考资料:
--   [1] IBM Db2 Documentation - Locking
--       https://www.ibm.com/docs/en/db2/11.5?topic=concurrency-locking
--   [2] IBM Db2 Documentation - LOCK TABLE
--       https://www.ibm.com/docs/en/db2/11.5?topic=statements-lock-table
--   [3] IBM Db2 Documentation - Isolation Levels
--       https://www.ibm.com/docs/en/db2/11.5?topic=levels-isolation
--   [4] IBM Db2 Documentation - Lock Monitoring
--       https://www.ibm.com/docs/en/db2/11.5?topic=monitoring-locks
...
-- 1. Db2 支持行级锁、页级锁和表级锁
-- 2. 锁可能自动升级（行锁 -> 表锁）
-- 3. 使用 LOCKLIST/MAXLOCKS 配置锁内存
-- 4. 不支持 advisory locks
-- 5. SKIP LOCKED DATA 从 Db2 9.7 开始可用
