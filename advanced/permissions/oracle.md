...
> 参考资料:
> - [Oracle SQL Language Reference - GRANT](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/GRANT.html)
> - [Oracle Database Security Guide](https://docs.oracle.com/en/database/oracle/oracle-database/23/dbseg/)

## Oracle 权限体系概览

Oracle 有三种权限类型:
### 系统权限: 操作数据库对象的能力（CREATE TABLE, CREATE SESSION 等）

### 对象权限: 操作特定对象的能力（SELECT ON users, INSERT ON orders 等）

### 角色: 权限的命名集合（可嵌套）

## 用户创建（user = schema，Oracle 独有的绑定关系）
...
### Oracle 200+ 系统权限是最细粒度的，但也最复杂。最小可行方案: 表级 GRANT。
