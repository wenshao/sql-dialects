...
> 参考资料:
> - [SQL Server T-SQL - Transactions](https://learn.microsoft.com/en-us/sql/t-sql/language-elements/transactions-transact-sql)
...
### DDL 是事务性的: CREATE/ALTER/DROP 可以回滚（Oracle 不行!）

### 认真考虑 RCSI: 大多数应用开启 RCSI 后并发性能显著提升

5. NOLOCK 不是性能优化手段，是数据一致性炸弹
### 事务尽量短: 长事务 + 锁 = 阻塞链 → 整个系统停摆

## 横向对比: SQL Server vs 其他方言的事务机制

### SNAPSHOT 隔离 vs 其他数据库的 MVCC（SQL Server 最核心的差异）
...
### WITH (NOLOCK) 文化及其危险（SQL Server 社区的特殊现象）
...
### IDENTITY vs SEQUENCE 及与其他数据库的对比
...
### MERGE 语句的 Bug（SQL Server 特有问题，专家建议避免使用）
...
### Oracle 自治事务（Oracle 独有，SQL Server 没有等价功能）
...
### Oracle NUMBER vs SQL Server 数值类型
...
### Oracle Flashback（Oracle 独有，SQL Server 有部分替代）
