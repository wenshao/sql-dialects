...
--   [4] MySQL 8.0 Reference Manual - ALTER TABLE
...
-- ALGORITHM 选项: INSTANT（最快）/ INPLACE / COPY（最慢）
...
-- 7. 最佳实践
-- ============================================================
-- 1. shardkey 在建表时确定，后续不可变更 → 谨慎选择
-- 2. 使用 ALGORITHM=INSTANT 避免锁表（适用于末尾 ADD COLUMN）
-- 3. 批量 DDL 操作在低峰期执行（减少对在线业务的影响）
-- 4. 需要 shardkey 变更时，使用 "创建新表 → 迁移 → 切换" 流程
-- 5. 广播表适合频繁 ALTER 的小表（维度表等）
-- 6. MODIFY COLUMN 操作确保所有分片的数据与新类型兼容
