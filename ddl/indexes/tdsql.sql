...
--   [4] MySQL 8.0 Reference Manual - CREATE INDEX
...
-- SHARDKEY 指定 user_id 为分片键，相同 user_id 的数据落在同一分片
...
-- ============================================================
-- 3. 全局二级索引（Global Secondary Index, GSI）
-- ============================================================
...
-- 6. 最佳实践
-- ============================================================
-- 1. shardkey 选择: 选择高基数、均匀分布、查询频率高的列
-- 2. 避免在低基数列上建 shardkey（如 status 只有几个值 → 数据倾斜）
-- 3. 高频查询条件包含 shardkey 可以大幅减少扫描范围
-- 4. GSI 用于需要跨分片查询但又不频繁更新的场景
-- 5. 广播表适合小维度表（如配置表），全分片冗余避免跨分片 JOIN
