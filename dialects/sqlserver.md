...
| 模块 | 简评 |
|---|---|
| [建表](../ddl/create-table/sqlserver.sql) | 聚集索引=物理排序，IDENTITY，Temporal Tables(2016+) 自动历史版本 |
| [改表](../ddl/alter-table/sqlserver.sql) | DDL 不可回滚（同 Oracle），在线操作需 Enterprise Edition |
| [索引](../ddl/indexes/sqlserver.sql) | Columnstore(2012+) 列存先驱，Filtered Index，INCLUDE 覆盖列 |
| [约束](../ddl/constraints/sqlserver.sql) | WITH CHECK/NOCHECK 可暂停约束检查，灵活但易滥用 |
| [视图](../ddl/views/sqlserver.sql) | Indexed View 物化视图，SCHEMABINDING 保护依赖 |
| [序列与自增](../ddl/sequences/sqlserver.sql) | IDENTITY+SEQUENCE(2012+)，SCOPE_IDENTITY() 获取自增值 |
| [数据库/Schema/用户](../ddl/users-databases/sqlserver.sql) | DENY 显式拒绝权限独有，AD 集成身份验证 |

### Advanced — 高级特性

| 模块 | 简评 |
|---|---|
| [动态 SQL](../advanced/dynamic-sql/sqlserver.sql) | sp_executesql 参数化查询防注入，比 EXEC() 安全 |
| [错误处理](../advanced/error-handling/sqlserver.sql) | TRY...CATCH 不能嵌套 TRANSACTION 回滚点，需配合 XACT_ABORT |
| [执行计划](../advanced/explain/sqlserver.sql) | 图形化执行计划+Live Query Statistics 实时监控 |
| [锁机制](../advanced/locking/sqlserver.sql) | 默认锁式并发，锁升级行→页→表，WITH(NOLOCK) 文化盛行 |
| [分区](../advanced/partitioning/sqlserver.sql) | Partition Function+Scheme 设计清晰，但需 Enterprise Edition |
| [权限](../advanced/permissions/sqlserver.sql) | DENY 独有（优先级高于 GRANT），Dynamic Data Masking(2016+) |
| [存储过程](../advanced/stored-procedures/sqlserver.sql) | T-SQL 过程式完整，Natively Compiled(Hekaton) 极致性能 |
| [临时表](../advanced/temp-tables/sqlserver.sql) | #temp 本地+##temp 全局+表变量 @t，tempdb 是核心资源 |
| [事务](../advanced/transactions/sqlserver.sql) | XACT_ABORT 必须设 ON，默认锁式并发，RCSI 推荐开启 |
| [触发器](../advanced/triggers/sqlserver.sql) | 无 BEFORE 触发器，只有 AFTER+INSTEAD OF |

### DML — 数据操作

| 模块 | 简评 |
|---|---|
| [删除](../dml/delete/sqlserver.sql) | OUTPUT DELETED.* 返回被删行，TOP(N) 限制删除行数 |
| [插入](../dml/insert/sqlserver.sql) | OUTPUT INSERTED.* 返回含自增值，BULK INSERT 批量导入 |
| [更新](../dml/update/sqlserver.sql) | OUTPUT INSERTED/DELETED 同时获取更新前后值 |
| [Upsert](../dml/upsert/sqlserver.sql) | MERGE 有已知竞态条件 Bug，专家建议避免使用 |

### Functions — 内置函数

| 模块 | 简评 |
|---|---|
| [聚合函数](../functions/aggregate/sqlserver.sql) | STRING_AGG 2017 才有，之前需 FOR XML PATH 黑魔法 |
| [条件函数](../functions/conditional/sqlserver.sql) | IIF(2012+) 简洁条件，CHOOSE 按位置选值 |
| [日期函数](../functions/date-functions/sqlserver.sql) | DATEADD/DATEDIFF/EOMONTH 语义清晰，FORMAT() 依赖 CLR 性能差 |
| [数学函数](../functions/math-functions/sqlserver.sql) | GREATEST/LEAST 2022 才有，之前需 CASE 模拟 |
| [字符串函数](../functions/string-functions/sqlserver.sql) | STRING_SPLIT(2016+)，CONCAT_WS，TRANSLATE(2017+) |
| [类型转换](../functions/type-conversion/sqlserver.sql) | TRY_CAST/TRY_CONVERT 安全转换先驱，失败返回 NULL |

### Query — 查询

| 模块 | 简评 |
|---|---|
| [CTE](../query/cte/sqlserver.sql) | WITH 标准支持，MAXRECURSION 防无限递归 |
| [全文搜索](../query/full-text-search/sqlserver.sql) | CONTAINS/FREETEXT+全文索引，需配置 Full-Text Catalog |
| [连接查询](../query/joins/sqlserver.sql) | CROSS APPLY/OUTER APPLY（比 LATERAL 更早），WITH(NOLOCK) 文化 |
| [分页](../query/pagination/sqlserver.sql) | OFFSET...FETCH(2012+) 标准语法，TOP WITH TIES |
| [行列转换](../query/pivot-unpivot/sqlserver.sql) | 原生 PIVOT/UNPIVOT，列名需静态指定 |
| [集合操作](../query/set-operations/sqlserver.sql) | UNION/INTERSECT/EXCEPT 完整支持 |
| [子查询](../query/subquery/sqlserver.sql) | 优化器自动展开关联子查询，Adaptive Join(2017+) |
| [窗口函数](../query/window-functions/sqlserver.sql) | Batch Mode(2019+) 性能提升 2-10x，无 NTH_VALUE |

### Scenarios — 实战场景

| 模块 | 简评 |
|---|---|
| [日期填充](../scenarios/date-series-fill/sqlserver.sql) | 无 generate_series，需递归 CTE 或数字表模拟 |
| [去重](../scenarios/deduplication/sqlserver.sql) | ROW_NUMBER+CTE DELETE 直接去重，语法简洁 |
| [区间检测](../scenarios/gap-detection/sqlserver.sql) | 窗口函数+数字表填充 |
| [层级查询](../scenarios/hierarchical-query/sqlserver.sql) | WITH RECURSIVE+hierarchyid 数据类型独有 |
| [JSON 展开](../scenarios/json-flatten/sqlserver.sql) | OPENJSON+CROSS APPLY 展开，FOR JSON 生成 |
| [迁移速查](../scenarios/migration-cheatsheet/sqlserver.sql) | T-SQL 偏离标准大，Collation 差异是迁移痛点 |
| [TopN 查询](../scenarios/ranking-top-n/sqlserver.sql) | TOP WITH TIES 独有，CROSS APPLY 分组 TopN |
| [累计求和](../scenarios/running-total/sqlserver.sql) | Batch Mode(2019+) 窗口函数性能飞跃 |
| [缓慢变化维](../scenarios/slowly-changing-dim/sqlserver.sql) | Temporal Tables(2016+) 系统自动维护历史版本 |
| [字符串拆分](../scenarios/string-split-to-rows/sqlserver.sql) | STRING_SPLIT(2016+) 原生支持，enable_ordinal(2022+) |
| [窗口分析](../scenarios/window-analytics/sqlserver.sql) | Batch Mode 大幅提速，GROUPS 帧和 NTH_VALUE 缺失 |

### Types — 数据类型

| 模块 | 简评 |
|---|---|
| [复合类型](../types/array-map-struct/sqlserver.sql) | 无原生数组，需 Table-Valued Parameter 或 JSON 模拟 |
| [日期时间](../types/datetime/sqlserver.sql) | 旧 datetime 精度仅 3.33ms，datetime2 100ns 精度 |
| [JSON](../types/json/sqlserver.sql) | JSON 存为 NVARCHAR 无专用类型，OPENJSON 解析，无 JSON 索引 |
| [数值类型](../types/numeric/sqlserver.sql) | DECIMAL/NUMERIC 精确，MONEY 类型便捷但有精度陷阱 |
| [字符串类型](../types/string/sqlserver.sql) | Collation 影响比较和索引，不同 Collation 列 JOIN 报错 |
