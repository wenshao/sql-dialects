...
--   [3] MySQL 8.0 Reference Manual - InnoDB Multi-Versioning
...
-- ============================================================
-- 2. DELETE vs TRUNCATE: 内部机制深度对比
-- ============================================================

-- 2.1 DELETE FROM users; -- 删除所有行
-- 执行过程:
--   (1) 逐行扫描聚集索引（全表扫描）
--   (2) 每行: 对索引记录加 X 锁（排他锁）
...
-- ============================================================
-- 5. 横向对比: 各引擎的 DELETE 实现策略
-- ============================================================

-- 5.1 标记删除 + purge（MVCC 引擎的主流方案）
-- MySQL InnoDB:
...
-- 5.2 不可变数据 + Compaction（LSM-Tree 和列式引擎）
-- ClickHouse:
...
-- 5.3 段级操作（传统 RDBMS 的 TRUNCATE 方案）
-- Oracle:
