...
SET u.status = 1
WHERE o.amount > 1000;
--     如果 users 和 orders 的 shardkey 相同（推荐）:
--       同一分片上的 JOIN 可以本地执行，性能好
--     如果 shardkey 不同:
--       需要跨分片数据传输 + 分布式事务，性能差
--
-- (3) 性能影响:
--     跨 N 个分片 = N 次网络 RTT
--     建议尽量让 UPDATE 在单分片内完成
...
-- ============================================================
-- 9. 横向对比: TDSQL vs 单机 MySQL UPDATE
-- ============================================================
