...
--   [2] Materialize Source Envelopes
...
-- 1. Materialize 不支持 INSERT ... ON CONFLICT（与 PostgreSQL 不兼容）
-- 2. 不支持 MERGE 语句
-- 3. TABLE 仅支持 INSERT，不支持 UPDATE / DELETE（流处理引擎定位）
-- 4. SOURCE 通过 ENVELOPE UPSERT 实现 Kafka 消息级别的 UPSERT
-- 5. 物化视图自动增量维护，无需手动刷新
-- 6. 如需表级 UPSERT，建议使用 PostgreSQL 作为上游，通过 CDC 同步
