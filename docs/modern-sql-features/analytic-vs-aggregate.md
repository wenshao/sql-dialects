...
### SQL:2016 — FILTER 落地到窗口

SQL:2016 标准化 `FILTER (WHERE ...)` 子句，并明确允许在窗口聚合上使用：
...
SQL:2016 标准允许 `agg_func(...) FILTER (WHERE cond) OVER (...)`。
...
`FILTER (WHERE cond)` 在 SQL:2016 中被允许用于窗口聚合，PostgreSQL 12+ 是第一个完整实现。
...
- SQL:2003 标准: ISO/IEC 9075-2:2003, Section 6.10 (window function), Section 10.9 (set function)
- SQL:2011 标准: ISO/IEC 9075-2:2011, Window functions enhancements (RANGE/GROUPS, EXCLUDE)
- SQL:2016 标准: ISO/IEC 9075-2:2016, FILTER clause on window aggregate
