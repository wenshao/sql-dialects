...
| Spark SQL | 是 | 3.3+ `spark.sql.optimizer.runtime.bloomFilter.enabled`（需显式开启，默认 false） |
...
Spark 3.0+ 引入 AQE（Adaptive Query Execution），3.3 进一步新增了运行时布隆过滤器（SPARK-32268 / SPARK-36792），用于在 shuffle 之前裁剪 fact 表。需要注意的是，`spark.sql.optimizer.runtime.bloomFilter.enabled` 在 3.3 / 3.4 / 3.5 中**默认值都是 `false`**——必须显式开启才能生效。容易被混淆的是 Dynamic Partition Pruning（`spark.sql.optimizer.dynamicPartitionPruning.enabled`）：DPP 自 3.0 起默认开启，但它只能裁剪**分区列**；运行时布隆过滤器则可用于任意连接键，但需要用户主动打开：
...
4. **运行时布隆过滤器已成为现代 OLAP 的事实标准**：30+ 引擎实现了 Dynamic Filtering / Runtime Filter / Runtime Bloom Filter：
   ...
   - **Spark SQL 3.3（2022）引入 `BloomFilterAggregate`**，基于 AQE 注入，但 `spark.sql.optimizer.runtime.bloomFilter.enabled` 仍默认 `false`，需用户显式开启（默认开启的是 DPP）
