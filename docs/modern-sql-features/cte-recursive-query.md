...
| Snowflake | ✅ | ✅ | 必须 | GA | - |
| ClickHouse | ✅ | ❌ | N/A | 20.6+ | 不支持递归 |
| Hive | ✅ | ❌ | N/A | 0.13+ | 不支持递归 |
| Spark SQL | ✅ | ✅ | 必须 | 3.4+ (2023) | 3.4 前不支持递归 |
...
| Trino | ✅ | ✅ | 必须 | 340+ | 有深度限制 |
| Presto | ✅ | ✅ | 必须 | 0.246+ | - |
| Databricks | ✅ | ✅ | 必须 | Runtime 13+ | 同 Spark SQL 3.4+ |
...
**关键发现**: OLAP / 大数据引擎（ClickHouse、Hive、StarRocks、Doris、MaxCompute、Redshift、Vertica 等）普遍不支持递归 CTE。这不是疏忽，而是设计选择——递归查询本质上是迭代计算，与大规模并行处理（MPP）架构的批量扫描模式不兼容。
...
-- PostgreSQL, MySQL, SQLite, BigQuery, Snowflake, TiDB, DuckDB 等
...
-- Oracle, SQL Server, Db2, SAP HANA, H2
WITH ancestors AS (
    SELECT id, parent_id, name FROM nodes WHERE id = 100
    UNION ALL
    SELECT n.id, n.parent_id, n.name
...
| Trino | ✅ | ✅ | - |
| Db2 | ✅ | ✅ | - |
| Spark SQL | ✅ | ❌ | 仅 UNION ALL |

MySQL 和 TiDB 仅支持 `UNION ALL` 是重要的兼容性差异。这意味着在这些引擎上遍历图结构时，必须手动维护访问路径来防止无限循环。
...
| SQLite | 1000 | `PRAGMA max_recursive_depth = N` 或编译时 | 连接级 |
...
| Db2 | 无限制 | 无内置参数 | - |
| Spark SQL | 100 | `spark.sql.cte.maxRecursionDepth` | 应用级 |
...
- SQL:2011 标准: CYCLE clause, SEARCH clause
