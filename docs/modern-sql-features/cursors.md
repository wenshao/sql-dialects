...
游标是过程化 SQL 中逐行处理查询结果集的核心机制。在集合导向的 SQL 世界里，游标提供了一种命令式的"一次一行"访问模式，弥合了声明式查询与命令式控制流之间的鸿沟。尽管现代 SQL 引擎不断增强集合操作能力，游标在存储过程、复杂业务逻辑处理、客户端分页以及遗留系统维护中仍然不可替代。SQL 标准从 SQL:1992 开始定义游标语义（包括 SCROLL、INSENSITIVE），SQL:1999 进一步扩展了 WITH HOLD 和 SENSITIVE/ASENSITIVE 灵敏度模型，至今仍是 RDBMS 过程化编程的基础设施之一。
...
| CockroachDB | ✅ | ✅ | ✅ (PL/pgSQL, v23.2+) | 23.2+ |
| TiDB | ✅ | ✅ | ✅ (存储过程) | 6.5+ |
...
| CockroachDB | ❌ | ✅ | NO SCROLL（仅前向） | 23.2+ |
| TiDB | ❌ | ❌ | 仅 NEXT | — |
| OceanBase | ❌ | ❌ | 仅 NEXT | — |
...
| Firebird | ✅(3.0+) | - | 3.0 前仅 NEXT，3.0+ 支持 SCROLL | 3.0+ |
...
| MariaDB | ❌ | ❌ | 不适用 | 同 MySQL |
| Oracle | ✅ (隐含) | ❌ | 读游标 COMMIT 后保持 | 读游标在 COMMIT 后可继续 FETCH；FOR UPDATE 游标在 COMMIT 时因行锁释放而失效 (非标准 WITH HOLD 语义) |
| SQL Server | ❌ | ❌ | 默认 COMMIT 后保持开启 | SET CURSOR_CLOSE_ON_COMMIT ON 可改变；默认 OFF，即 COMMIT 后游标不自动关闭 |
...
| SQL Server | ✅ | ✅ (ADO/ODBC) | 客户端 | T-SQL DECLARE CURSOR 为服务端游标；API cursors (ADO/ODBC) 为客户端游标 |
...
| CockroachDB | ✅ `refcursor` | ✅ (v23.2+) | ✅ | PG 兼容 (23.2+) |
| Firebird | ❌ | ❌ | ❌ | 游标名必须静态 |
...
BEGIN
    ...
    -- 注意：PL/pgSQL 的 FETCH 只能单行，需使用 LOOP 遍历所有行
    LOOP
        FETCH dyn_cur INTO rec;
        EXIT WHEN NOT FOUND;
        -- 处理 rec
    END LOOP;
    CLOSE dyn_cur;
...
FAST_FORWARD  INSENSITIVE  前向只读优化      最快          简单遍历(推荐)
              (内部 STATIC                              FORWARD_ONLY + READ_ONLY
               优化)                                     的显式组合类型
```
...
CockroachDB 从 v23.2 开始引入 PL/pgSQL 游标支持。

```sql
-- CockroachDB PL/pgSQL 游标 (v23.2+)
