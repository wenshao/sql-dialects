...
│ Oracle              │ ✓        │ Oracle Text        │ CONTAINS     │ ✓ 内置   │
│ SQLite              │ ✓        │ FTS5 虚拟表        │ MATCH        │ 自定义   │
│ Elasticsearch SQL   │ ✓        │ Lucene 倒排索引    │ MATCH/QUERY  │ ✓ 插件   │
├─────────────────────┼──────────┼────────────────────┼──────────────┼──────────┤
│ Snowflake           │ 有限     │ SEARCH OPTIMIZATION│ SEARCH       │ 有限     │
...
-- 方案 5: pg_cjk_parser (PostgreSQL 16+)
...
Snowflake 的全文检索能力较新，侧重于大规模数据上的文本搜索。
...
SELECT * FROM articles WHERE body MATCH_REGEXP '(database|DB)\s+optim'; -- 正则
...
├────────────────────┼──────────────┼──────────────┼──────────────┼──────────────┼──────────┤
│ 基本搜索           │ SEARCH()     │ SEARCH()     │ hasToken()   │ MATCH        │ MATCH    │
│ AND                │ 空格 (隐含)  │ 空格 (隐含)  │ AND          │ MATCH_ALL    │ 默认     │
│ OR                 │ ✗            │ ✗            │ OR           │ MATCH_ANY    │ ✗        │
│ NOT                │ ✗            │ -            │ NOT          │ ✗            │ ✗        │
...
│                     │ GiST (R-tree)     │ 有损压缩, 需要 recheck, 支持排序             │
├─────────────────────┼───────────────────┼──────────────────────────────────────────────┤
...
│ SQL Server       │ Okapi BM25 (CONTAINSTABLE RANK)           │
