...
|------|-----------|----------|---------|------------|----------|--------------|----------|
| PostgreSQL | 是 | 是 | 是 | 是 | 是 (9.2+) | 是 (9.5+) | 是 |
...
`anyelement` 系列实现了简洁的"参数化多态"，比 SQL 标准更早、更彻底。`anycompatible` (9.5+) 进一步引入了"统一类型推导"，处理 `coalesce(int, bigint)` 这类多源参数。其他引擎要么用动态类型 (`VARIANT`/`ANY`) 模拟，要么放弃。
