...
**图例**：✅ = 允许隐式转换（表达式上下文自动触发） | ❌ = 不允许（需要显式 CAST 或完全不支持）

> **说明**: 本文的矩阵统一标注**表达式上下文**的隐式转换（即在 SELECT/WHERE/JOIN 等表达式中是否自动转换）。部分引擎（如 PostgreSQL）在 INSERT/UPDATE 赋值上下文中允许更多隐式转换（如 `bigint → int`、`timestamp → date`），这些赋值级转换未标注为 ✅。各引擎的赋值级规则详见对应章节的说明。
...
- **NUMERIC → FLOAT8 = ❌**（赋值级 castcontext='a'，非隐式）；**FLOAT8 → NUMERIC = ❌**（同为赋值级）
...
- **BOOLEAN → VARCHAR/VARIANT = ✅**: `TRUE` → `'true'`。BOOLEAN → FLOAT/NUMBER = ❌（需显式 CAST）
- **DATE → TIMESTAMP_NTZ/LTZ = ✅**: 添加午夜时间
- **时间类型 → VARCHAR = ✅**: 所有时间类型可隐式转为字符串
- **VARCHAR → 数字 = ❌**: 不像 MySQL/Oracle 那样隐式转换
- **NUMBER ↔ FLOAT = ✅**: 双向隐式（Snowflake 标注为 Coercible）。**精度风险**: NUMBER 精度可达 38 位，FLOAT（DOUBLE）仅约 15-17 位有效数字，超过 15 位的 NUMBER 转 FLOAT 都会丢失精度
...
- **提升链（基于源码 `TypeInfoUtils.java` 排名）**: `TINYINT(1) → SMALLINT(2) → INT(3) → BIGINT(4) → DECIMAL(5) → FLOAT(6) → DOUBLE(7) → STRING(8)`。注意 DECIMAL 排名低于 FLOAT，因此 FLOAT/DOUBLE → DECIMAL = ❌
- **STRING → DOUBLE = ✅（特例）**: 源码中硬编码的 STRING_GROUP→DOUBLE 特例。但 STRING → DECIMAL = ❌（无特例）
...
| From ↓ \ To → | INT2 | INT4 | INT8 | FLOAT | DECIMAL | STRING | BYTES | DATE | TIMESTAMP | TIMESTAMPTZ | BOOL | JSONB | UUID |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **INT2** | ✅ | ✅ | ✅ | ✅ | ✅ | A | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **INT4** | A | ✅ | ✅ | ✅ | ✅ | A | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **INT8** | A | A | ✅ | ✅ | ✅ | A | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
- **与 PostgreSQL 严格度相当，但策略不同**: 设计团队明确表示"nothing good comes out of a language that allows implicit type coercions"。CockroachDB 不是简单地"更严格"，而是用**多态字面量**替代隐式转换（`42` 被解析为 INT/FLOAT/DECIMAL 而非类型转换），并用**更广泛的赋值级转换**覆盖 INSERT/UPDATE 场景
- **INT2/INT4/INT8 是 16/32/64 位整数**: CockroachDB 中 `INT` 默认解析为 `INT8`（64 位），可通过 session variable `default_int_size` 改为 `INT4`（32 位）。矩阵使用 INT2/INT4/INT8 明确区分位宽，避免 `INT` 的歧义
...
- **数字族隐式提升**: `INT2→INT4→INT8→DECIMAL` 和 `INT→FLOAT` 为隐式，`FLOAT→INT` 和 `FLOAT→DECIMAL` 仅在赋值上下文
...
**与 Hive 的差异**: MaxCompute 支持 DATE→DATETIME→TIMESTAMP 隐式提升链（Hive 的 DATE↛TIMESTAMP）。FLOAT/DOUBLE→DECIMAL = ❌（同 Hive 源码逻辑）。BOOLEAN 和 BINARY 同样完全孤立。
...
**关键特点**: VARCHAR 在语法层面是"万能源"（全行 ✅），所有类型也都可隐式转为 VARCHAR。但**运行时会做严格的值验证**——VARCHAR→BOOLEAN 仅接受 `'TRUE'`/`'FALSE'`，VARCHAR→DATE 仅接受 `'yyyy-MM-dd'` 格式，不合法的值会直接报错（与 MySQL 的 `'abc'→0` 静默处理不同）。DATE↔TIMESTAMP 双向隐式。TRY_CAST（1.15+）失败返回 NULL。
...
**与 Spark ANSI 差异**: STRING→BIGINT/DOUBLE/BOOLEAN/DATE/TIMESTAMP 隐式（Spark ANSI 中全部 ❌）。支持函数调用时的隐式降级（DOUBLE→INT 等）。**上下文注意**: STRING 的跨类型转换主要发生在函数参数传递和比较运算中，`'42' + 1` 形式的算术运算在 ANSI 模式下仍可能报错。
...
| Databricks / Spark | `TRY_CAST(x AS type)` | Spark 3.4+（[SPARK-43088](https://issues.apache.org/jira/browse/SPARK-43088)）/ Databricks Runtime 11.2+ |
