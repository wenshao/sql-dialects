...
(9)  SQL Server: 有列存储索引 (Columnstore Index), 但其 segment elimination 是列存储内部优化, 与 ClickHouse 等引擎的用户定义跳数索引 (Data Skipping Index) 是不同机制
(10) SQLite: FTS5 虚拟表实现全文搜索
(11) BigQuery/EXASOL: 列存引擎, 无传统索引, 依赖自动优化
(12) Snowflake: 无用户可创建索引, 依赖 Search Optimization Service
(13) Snowflake: 支持 GEOGRAPHY/GEOMETRY 类型和空间函数, 但无用户可创建的空间索引
(14) ClickHouse: 主键排序代替 B-tree, MergeTree ORDER BY 即排序键
(15) ClickHouse: 全文通过 tokenbf_v1/ngrambf_v1 跳数索引实现
(16) Hive: ORC row group 索引 + Bloom Filter (非真正 Bitmap 索引)
(17) DuckDB: 使用 ART (Adaptive Radix Tree) 而非 B-tree, 但功能等效 (有序索引, 支持范围查询)
...
(25) StarRocks: 3.3.0+ 支持倒排索引 (Inverted Index)，用于加速全文检索和等值/范围查询
(26) Doris: 通过倒排索引 (INVERTED INDEX) 配合 parser 参数实现全文检索，非独立全文索引类型
```
...
B-tree 是绝大多数关系型数据库的默认索引结构。它支持等值查询、范围查询和排序操作，是最通用的索引类型。注意: 大多数引擎（如 MySQL InnoDB、PostgreSQL、Oracle）实际实现的是 B+tree（数据仅存储在叶子节点，叶子节点之间有链表连接），但习惯上统称为 B-tree。
...
-- Hive: ORC 文件格式内置 row group 索引 + Bloom Filter (非真正 Bitmap 索引, 不需要显式创建)
...
TiDB               ✅             CREATE INDEX IF NOT EXISTS (MySQL 超集)
OceanBase          ✅             CREATE INDEX IF NOT EXISTS (MySQL 超集)
...
引擎              支持    语法                 版本        限制
────────────────  ──────  ──────────────────   ──────────  ────────────────────
PostgreSQL         ✅     WHERE clause          7.2+        几乎无限制
SQL Server         ✅     WHERE clause          2008+       不能跨列 OR, 无函数
SQLite             ✅     WHERE clause          3.8.0+      条件受限
...
-- 优化器会自动匹配查询中的表达式; 需要收集索引统计信息 (DBMS_STATS)
...
MariaDB            ⚠️     Virtual Column + Index           5.2+ (间接支持)
DuckDB             ❌     不支持
BigQuery           N/A    无用户索引
