...
> **MariaDB 注**: MariaDB 10.2.3+ 也支持 `->>` 快捷运算符，等价于 `JSON_UNQUOTE(JSON_EXTRACT(...))`，即 `data->>'$.user.name'`。
| Oracle (点表示法) | `t.data.user` | `t.data.user.name.string()` | `t.data.scores[0]` |
...
| Spark SQL | `get_json_object(data, '$.user')` | `get_json_object(data, '$.user.name')` | `get_json_object(data, '$.scores[0]')` |
...
| StarRocks | `data->'user'` | `get_json_string(data, '$.user.name')` | `json_query(data, '$.scores[0]')` |
...
-- DuckDB 1.2.0+: 原生支持标准 JSON_TABLE 语法
SELECT jt.*
FROM orders o,
JSON_TABLE(
    o.data,
    '$.items[*]'
    COLUMNS (
        product  VARCHAR  PATH '$.product',
        qty      INT      PATH '$.qty',
        price    DOUBLE   PATH '$.price'
    )
) AS jt;

-- DuckDB < 1.2.0: unnest + json_extract
...
| DuckDB | `JSON_TABLE` | 支持 | 支持 | 支持 | 1.2.0+ |
