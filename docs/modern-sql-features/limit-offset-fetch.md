...
| TOP | `SELECT TOP n ...` | SQL Server 6.5 (1996) | 非标准，微软系 |
...
-- 12c+ 也支持 LIMIT（仅在 SQL*Plus 和部分驱动中）
-- 传统 ROWNUM 方案（12c 之前）
SELECT * FROM (
    SELECT t.*, ROWNUM rn FROM (
        SELECT * FROM orders ORDER BY id
...
-- OFFSET FETCH（2012+，必须有 ORDER BY 和 OFFSET）
SELECT * FROM orders ORDER BY id
OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY;

-- 仅跳过行（无 FETCH 则取剩余全部）
SELECT * FROM orders ORDER BY id OFFSET 20 ROWS;
```
...
| `FETCH FIRST 10 ROWS WITH TIES` | DENSE_RANK() OVER (...) + WHERE dr <= 10 | 无 WITH TIES 的引擎 |
