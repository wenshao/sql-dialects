...
| Oracle | 是 | 9i (2001) | 早期就支持，标准前已实现 |
| SQL Server | **否** | -- | 至今无显式语法，需 CASE WHEN |
| DB2 | 是 | LUW 8.2 / z/OS 9 | 完整支持 |
| Snowflake | 是 | GA | 与 PG/Oracle 阵营一致，ASC 默认 NULLS LAST |
...
| Snowflake | NULLS LAST | "NULL 最大"派（与 PG/Oracle 一致） |
...
> 关键观察：两派几乎五五分。"NULL 最大"派 ~18 家，"NULL 最小"派 ~31 家。Oracle/PostgreSQL/DB2 这三大老牌商业/开源库共同站在"NULL 最大"一侧，而 MySQL 阵营和大多数 Apache 系（Spark/Hive/Flink/Impala）站在"NULL 最小"一侧。
...
Oracle 是历史上最早实现 `NULLS FIRST/LAST` 的主流商业数据库（9i, 2001），SQL:2003 标准其实是事后追认了 Oracle 的实现。
...
### Snowflake（NULLS LAST 默认）

```sql
-- 默认 ASC NULLS LAST（与 PG/Oracle 一致，与 BigQuery 相反）
...
Snowflake 属于"NULL 最大"阵营，ASC 默认 NULLS LAST、DESC 默认 NULLS FIRST，与 PostgreSQL、Oracle、DB2 行为一致。需要注意的是，Snowflake **并未**提供类似 `DEFAULT_NULL_ORDERING` 的会话级参数来改变默认顺序——要覆盖默认行为，必须在每个 `ORDER BY` 中显式写出 `NULLS FIRST/LAST`。
...
DuckDB 与 PostgreSQL 阵营一致（NULL 视为最大），并额外提供了 `default_null_order` 会话级开关——这在主流引擎中相当罕见。
...
- **NULL 最大派**（ASC NULLS LAST）：PostgreSQL、Oracle、DB2、DuckDB、Trino/Presto、Snowflake、Redshift 等
...
- **会话参数开关**：参考 DuckDB 的 `default_null_order`，对迁移用户友好（注意：Snowflake 并没有这样的参数，必须在每个 ORDER BY 中显式指定）
...
| Oracle | NULLS LAST | NULLS FIRST | 是 (9i+) | 部分 | NULL 最大 |
...
| Snowflake | NULLS LAST | NULLS FIRST | 是 | -- | NULL 最大 |
