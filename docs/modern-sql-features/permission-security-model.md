...
SQL Server       ✓            ✓          ✓        ✓        ✓        ✓       ✓
SQLite           ✗            ✗          ✗        ✗        ✗        ✗       无权限系统(应用层控制)
BigQuery         ✓(IAM+SQL)   ✓(IAM)     ✓(*)     ✓(IAM)   ✓(IAM)   ✗       IAM 条件绑定
Snowflake        ✓            ✓          ✗(*)     ✓        ✓        ✗       ✗(使用角色层级)
ClickHouse       ✓            ✓          ✓        ✗(*)     ✓        ✗       ✓
Hive             ✓            ✓          ✓(*)     ✗        ✓        ✗       ✓
Spark SQL        ✓            ✓          ✓(*)     ✗        ✓        ✗       ✓
...
2. Oracle: 支持列级 GRANT (INSERT/UPDATE/REFERENCES), SELECT 需 VPD/Data Redaction
3. SQL Server: 唯一支持 DENY 的主流引擎, DENY 优先于 GRANT
4. SQLite: 嵌入式数据库, 无多用户权限系统
4b. DuckDB 0.9+: 支持完整 RBAC (CREATE USER, CREATE ROLE, GRANT/REVOKE)
5. BigQuery: 支持 SQL-level GRANT 映射到 IAM (GRANT/REVOKE 语法)
...
BigQuery         ✓(IAM+SQL)  ✓(IAM)   ✗        ✗        预定义 IAM 角色    GCP IAM 管理
...
StarRocks       Row Policy            3.0+       CREATE ROW POLICY              ★★★
Doris           Row Policy            2.0+       CREATE ROW POLICY              ★★★
DuckDB          不支持                 -          -                              -
...
-- 一个列只能绑定一个 Row Access Policy, 但一个表可以有多个 (绑定到不同列)
-- 解绑:
