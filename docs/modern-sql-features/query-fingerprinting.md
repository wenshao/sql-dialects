...
| MySQL | Performance Schema 内存 | 否（无持久化） | 200 条/用户（`max_digest_length`） | 重启清空 |
...
| 版本 | 关键变化 |
|------|---------|
| 5.6 (2013) | Performance Schema 引入 `events_statements_summary_by_digest`，使用 SHA-1 |
...
`DIGEST_TEXT` 是逐 token 拼接的规范化文本，`DIGEST` 是它的 SHA-256（早期版本是 SHA-1）。
...
- MySQL 默认 `performance_schema_digests_size = 200`，**严重偏低**，是常见踩坑
