...
|------|------|---------|---------|---------|
| SQL Server | `ALTER TABLE/INDEX ... REBUILD WITH (RESUMABLE=ON)` | `ALTER INDEX ... PAUSE` | `ALTER INDEX ... RESUME` | 2017 RTM / Azure SQL Database (2017) |
...
> SQL Server 2017 RTM 引入的 RESUMABLE 是同类功能中最完整的：可手动 PAUSE/RESUME，可设置 `MAX_DURATION` 自动暂停（达到时间窗口后），可在故障重启后从断点继续。Oracle 的 RESUMABLE 是另一种语义——遇到 ORA-1652 类资源不足错误时挂起等待，而非用户主动控制。
...
SQL Server 自 2005 企业版开始支持 `WITH (ONLINE = ON)`，是除 Oracle 外最早进入"在线维护"工业化阶段的引擎。2017 RTM (Oct 2017) 进一步引入了**可暂停**的索引/表重建，把该领域推到一个新高度。
...
-- RESUMABLE 重建（2017 RTM / Azure SQL）
...
SQL Server 2017 RTM 引入的 RESUMABLE 是同类功能中最完整的实现：可手动 PAUSE、RESUME、ABORT，可设置 MAX_DURATION 自动暂停，故障重启后从断点继续。
...
| SQL Server | REBUILD ONLINE (2005+) | RESUMABLE (2017 RTM+) | -- | 部分（Auto-Tune） |
