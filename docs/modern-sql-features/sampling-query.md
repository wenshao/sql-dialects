...
| Impala | `TABLESAMPLE SYSTEM` | -- | 是 | -- | -- | 是 | 2.0+ |
...
> 统计：约 25 个引擎支持某种形式的采样语法，约 20 个引擎完全不支持或需要替代方案模拟。
...
```sql
-- PostgreSQL / SQL Server / BigQuery / Vertica / SAP HANA
...
-- 子查询上采样
SELECT * FROM (SELECT * FROM orders WHERE status = 'completed') sub
...
```sql
-- BigQuery（仅 SYSTEM，按存储块采样，不支持 BERNOULLI/REPEATABLE）
SELECT * FROM dataset.large_table TABLESAMPLE SYSTEM (10 PERCENT);
...
-- 先采样再过滤（标准语义：TABLESAMPLE 在 FROM 中）
SELECT * FROM orders TABLESAMPLE BERNOULLI(10)
...
| RESERVOIR | -- | -- | -- | -- | -- | 是 | -- | -- | -- | -- |
| 分层采样 | 模拟 | 模拟 | 模拟 | 模拟 | LIMIT BY | 模拟 | 桶 | 模拟 | 原生 | 模拟 |
| 按数据量采样 | -- | -- | -- | -- | -- | -- | 是 | -- | -- | -- |
...
| 超大表快速预览 | BigQuery/Snowflake SYSTEM | 块级跳过，秒级响应 |
