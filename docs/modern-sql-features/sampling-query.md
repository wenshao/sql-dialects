...
| BigQuery | `WHERE RAND() < 0.1` 或 `ORDER BY RAND() LIMIT N` | 全表扫描 |
| Flink SQL | 不适用（流处理） | -- |
...
```sql
-- BigQuery（不支持 TABLESAMPLE，用 RAND() 替代）
SELECT * FROM dataset.large_table WHERE RAND() < 0.1;  -- 约 10%
...
| Trino | -- | 不支持 |
| BigQuery | -- | 不支持（TABLESAMPLE 本身不支持） |
| Redshift | -- | 不支持 |
