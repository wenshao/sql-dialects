...
| SQL Server | `EQ_ROWS` | `1 / DENSITY` | DBCC SHOW_STATISTICS | 本地变量等值用 density (≈1/NDV)；不等值用 30% |
...
-- 优化器不知道 @status 的具体值
-- 对等值谓词 (= @var)：LegacyCE 与 NewCE 都使用 density (≈ 1/NDV)
-- 对不等值谓词 (>, <, BETWEEN @var)：使用 30% 常量 (LegacyCE) 或 NewCE 的 30%/16.4% 等启发式
-- 即 Status = @status 实际估计为 density (≈ 1/NDV)，而非 30%

DECLARE @price MONEY = 100;
SELECT * FROM Orders WHERE Price > @price;
-- 不等值 + 本地变量 → 30% 启发式（与实际分布无关）
...
- **MSCN** (CIDR 2019, Kipf et al.)：multi-set convolutional network, 训练 JOIN 选择性估计
