...
| MySQL | 间接 | `START TRANSACTION WITH CONSISTENT SNAPSHOT` | 同会话内事务一致；不可跨会话直接传 | 5.6 (2013) |
...
MySQL InnoDB 的 `START TRANSACTION WITH CONSISTENT SNAPSHOT` 在 5.6 (2013) 引入，提供了**会话内**的一致性快照，但**不能跨会话共享**。这就是 `mydumper`（2010 年由 Facebook MySQL 团队开源，后由 Domas Mituzas 和 Mark Leith 维护）解决的核心问题。
