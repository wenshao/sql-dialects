...
- Oracle 的 `DBMS_STATS.EXPORT_TABLE_STATS / IMPORT_TABLE_STATS / CREATE_STAT_TABLE` 自 9i (2001) 起即存在，是这一领域最完整的实现。
...
|------|---------|----------|------|----------|-----------|-------------|----------|
| Oracle | `EXPORT_TABLE_STATS` | expdp 元数据 | `IMPORT_TABLE_STATS` | `LOCK_TABLE_STATS` | 高（向后） | Data Pump | 9i+ (2001) |
...
| 引擎 | 锁定语法 | 粒度 | 解锁 | 默认 | 备注 |
|------|---------|------|------|------|------|
| Oracle | `DBMS_STATS.LOCK_TABLE_STATS` | 表/分区/schema/库 | `UNLOCK_*` | 否 | 9i+ |
...
Oracle 在 9i (2001) 推出 `DBMS_STATS` 包，将 8i 时代的 `ANALYZE TABLE COMPUTE STATISTICS` 完全替代。`DBMS_STATS` 不仅功能更强，而且自带完整的导出/导入/锁定/还原工作流：
...
Oracle 9i 起支持表级统计锁定，11g 起增加分区级锁定：
...
| Oracle 9i → 10g | 部分 | 9i 直方图格式与 10g 不同，可能需要重新收集 |
...
- Oracle 是少数能"统计随便跨主版本"的引擎，得益于 DBMS_STATS 在 9i 起就采用了稳定的存储格式。
...
Oracle 自 9i (2001) 起就提供了完整的"导出-导入-锁定-还原"工作流，覆盖表/分区/schema/database 各级粒度。其他引擎要么只支持其中一部分（PostgreSQL 18 仅支持导出/导入），要么完全不支持（MySQL）。这是 Oracle 在生产环境运维上 25 年积累下的核心竞争力之一。
...
如果只做"导出/导入"，能力是初级的；如果同时支持"锁定+手工设置"，能力是中级的；如果再支持"还原到任意时刻+保留历史"，能力才是高级的。Oracle 9i 起就实现了完整三层，新引擎不应该只做第一层就停下。
...
Oracle DBMS_STATS API 在 9i (2001) 到 23ai (2024) 的 23 年里保持兼容，是企业用户能放心做"导出 → 升级 → 导入"的根本原因。如果新引擎的统计存储格式每个主版本都变，那"统计导出/导入"就是空话——用户不会信任跨版本不兼容的能力。
