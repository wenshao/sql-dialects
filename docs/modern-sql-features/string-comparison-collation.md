...
| ClickHouse | `%` `_` | `\` | 支持 | **支持** | 22.4+ 支持 `ILIKE` |
| Trino | `%` `_` | 无默认 | 支持 | 不支持 | - |
...
**关键发现**: `ILIKE` 支持仅限 PostgreSQL、Snowflake、ClickHouse（22.4+）、Spark SQL（3.3+）和 DuckDB。其他方言需要 `LOWER(col) LIKE LOWER(pattern)` 或使用排序规则来实现大小写不敏感匹配。
