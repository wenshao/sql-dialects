...
将时间戳截断或对齐到固定间隔是时序分析最基础的操作。不同引擎提供三种主要方法：`DATE_TRUNC`（截断到日历边界）、`time_bucket`（对齐到任意间隔）、`DATE_BIN`（SQL 标准方向的尝试）。

### 支持矩阵

| 引擎 | DATE_TRUNC | time_bucket | DATE_BIN | 自定义间隔分桶 | 备注 |
|------|-----------|-------------|----------|--------------|------|
| PostgreSQL | 是 (8.1+) | -- | 是 (14+) | DATE_BIN | `DATE_TRUNC('hour', ts)` |
...
| BigQuery | 是 | -- | -- | `TIMESTAMP_BUCKET` | `TIMESTAMP_BUCKET(ts, INTERVAL 15 MINUTE)` (2024+) |
...
| ClickHouse | -- | -- | -- | `toStartOfInterval` | `toStartOfInterval(ts, INTERVAL 15 MINUTE)` |
...
-- BigQuery
SELECT TIMESTAMP_BUCKET(ts, INTERVAL 15 MINUTE) FROM readings;
-- 传统方式: TIMESTAMP_TRUNC(ts, HOUR) 仅支持日历边界
...
| CrateDB | 是 | `generate_series` + JOIN | -- | 内置 `generate_series` |
| Flink SQL | 是 | TUMBLE 窗口自动填充 | -- | 流处理语义，空窗口可输出 |
...
| Vertica | -- | -- | `TimeSeries` 子句 | **原生 TimeSeries 子句** |
| Impala | -- | -- | 应用层 | 无原生方案 |
...
-- ClickHouse (不支持 RANGE INTERVAL，使用子查询)
SELECT ts, value,
       avgIf(value, ts2 BETWEEN ts - INTERVAL 1 HOUR AND ts) AS ma_1h
FROM readings r1
CROSS JOIN readings r2;
-- 实际中更常用 arrayJoin 或窗口函数近似
...
| ClickHouse | 是 | `ASOF JOIN ... ON ... AND ts >= ts` | 18.6+ |
...
| BigQuery | TIMESTAMP_TRUNC / TIMESTAMP_BUCKET | `GROUP BY TIMESTAMP_TRUNC(ts, HOUR)` | 标准 |
...
| QuestDB | 是 | `ALTER TABLE ... SET PARAM maxUncommittedRows` | 部分自动 | 分区级 DROP |
...
| DB2 | 纳秒 (ns) | 微秒 | 是 | 是 | `TIMESTAMP(12)` 最大 |
...
> 注: 对于高频时序数据（金融 tick 数据、网络包分析），纳秒精度至关重要。支持纳秒的引擎有 Oracle、SQL Server、DB2、Snowflake、ClickHouse、Trino、InfluxDB、Flink SQL。
...
-- 1. 时间分桶 (TIMESTAMP_BUCKET, 2024 新增)
SELECT TIMESTAMP_BUCKET(ts, INTERVAL 15 MINUTE) AS bucket,
       sensor_id,
       AVG(temperature) AS avg_temp,
...
|------|------------|---------|----------|-----------|-----------|--------|-----------|---------|----------|
| 时间分桶 | time_bucket (原生) | SAMPLE BY (原生) | DATE_BIN (原生) | toStartOf* (原生) | DATE_TRUNC / DATE_BIN | time_bucket / DATE_BIN | TIME_SLICE | TIMESTAMP_BUCKET | TUMBLE |
