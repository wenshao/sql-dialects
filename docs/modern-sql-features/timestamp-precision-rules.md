...
| MySQL | `DATETIME(p)` / `TIMESTAMP(p)` | 0..6 | 0 | 1 微秒 | 5.6.4+ (2013) |
...
MySQL 直到 5.6.4（2013 年）才引入亚秒精度——在此之前所有时间戳都是整秒精度。这是 MySQL 长期落后于 PostgreSQL 的"知名痛点"。
...
6. **MySQL 历史包袱**：MySQL 5.6.4（2013）才支持亚秒精度，且默认精度为 0（整秒）——这是兼容老应用的妥协。这意味着 MySQL 的"DATETIME 自动有微秒"是错觉：不显式指定 `(6)` 仍是整秒。
