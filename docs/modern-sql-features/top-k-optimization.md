...
| Redshift | `LATERAL` (2018+) | 是 |
| DuckDB | `LATERAL` | 是 |
| ClickHouse | `ARRAY JOIN` (语义不同) | 用 LIMIT BY 代替 |
| Trino | `LATERAL` (v316+，with restrictions) | 部分 |
| Presto | 否 | -- |
...
7. **LATERAL JOIN 是"每组 Top-K"的圣杯**——PostgreSQL/Oracle/SQL Server/DB2/Snowflake/DuckDB/CockroachDB 等都正确实现。MySQL 8.0.14 才补上 `LATERAL`。**Trino 自 v316 (2019) 起支持 `LATERAL`，但有相当多限制**（例如不支持相关引用 + LIMIT 的某些组合），实际生产中仍常需用窗口函数代替。Presto 至今未跟进。
...
11. **覆盖索引（INCLUDE columns）+ Top-K 是 OLTP 高频查询的最后一公里**——PostgreSQL 11+、SQL Server 2005+ 原生支持 `INCLUDE` 子句；MySQL 没有 `INCLUDE` 语法，只能通过把额外列放进复合索引尾部实现覆盖索引（"covering index"），可以让"分页 + 排序"查询从几百 ms 降到亚毫秒级。
...
| LATERAL Top-K | 9.3+ | 8.0.14+ | 12c+ | CROSS APPLY | LIMIT BY | 是 | 是 | v316+ (受限) | LATERAL VIEW |
