...
SQL Server 还支持 `WITH CHECK OPTION`（仅 CASCADED 语义）和 `SCHEMABINDING` 选项（绑定视图与基表 schema，防止基表被改）。索引视图（indexed view）是 SQL Server 的物化视图实现，不能直接 DML，但优化器可以自动用它替代基表查询。
...
7. **WITH CHECK OPTION 的 LOCAL/CASCADED 区别在多层视图嵌套时才显现**：单层视图下二者等价。Oracle 和 SQL Server 只支持单一 CHECK OPTION（默认 CASCADED 语义），PostgreSQL/MySQL/DB2/MariaDB 支持完整的 LOCAL/CASCADED 区分。
