...
SQLite 3.24+ 和 DuckDB 0.9.0+ 也使用相同语法:
...
INSERT OVERWRITE 是分区级别的全量替换，而非行级 UPSERT。适用于 ETL 管道的幂等写入场景。详见 [insert-overwrite.md](insert-overwrite.md)。
...
| 表间批量同步 | 不支持 — 只能用 INSERT...SELECT | **更优** — USING source_table |
| 需要 DELETE 动作 | 不支持 | 支持 |
...
5. **MySQL ON DUPLICATE KEY 的 dead lock**: 批量 UPSERT 时，如果多个会话以不同顺序插入相同键值范围，可能产生死锁。保持插入顺序一致可以避免。
...
| SQL Server | `INSERT + UPDATE`（显式分支） | MERGE 有 bug，专家建议避免 |
| SQLite | `INSERT ... ON CONFLICT DO UPDATE` | 3.24+ 支持，语法同 PG |
| DuckDB | `INSERT ... ON CONFLICT DO UPDATE` | 0.9.0+ 支持，语法同 PG |
| BigQuery | `MERGE` | 唯一选择 |
...
| 跳过冲突行 (DO NOTHING) | `DO NOTHING` | `INSERT IGNORE` | 空 WHEN MATCHED | 不支持 |
