...
--   [3] MySQL 8.0 Reference Manual - Flow Control Functions
...
-- count=0 时返回 NULL（而非报错 Division by zero）
...
-- 1. CASE WHEN/IF/COALESCE 等在各分片独立执行，无跨分片问题
-- 2. CAST/CONVERT 中涉及日期字符串时，确保各分片时区一致
-- 3. 条件函数不涉及数据路由，性能不受分布式影响
-- 4. GROUP BY 中的 CASE WHEN 可能导致跨分片聚合（注意 shardkey 对齐）
