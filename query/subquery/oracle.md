...
> 参考资料:
> - [Oracle SQL Language Reference - Using Subqueries](https://docs.oracle.com/en/database/oracle/oracle-database/23/sqlrf/Using-Subqueries.html)
...
### WITH CHECK OPTION（视图/DML 子查询中确保数据满足条件）
...
Oracle 优化器的子查询转换策略:
### 子查询展开（Subquery Unnesting）: 将子查询转为 JOIN

   IN (subquery) → semi-join
   NOT IN (subquery) → anti-join
   EXISTS (subquery) → semi-join
### 视图合并（View Merging）: 将内联视图合并到外层查询

### 推入谓词（Predicate Pushing）: 将外层 WHERE 推入子查询
