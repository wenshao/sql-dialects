# Databricks SQL: 日期序列生成与间隙填充 (Date Series Fill)

> 参考资料:
> - [Databricks SQL Documentation](https://docs.databricks.com/en/sql/language-manual/functions/sequence.html)
...
使用 sequence + explode 生成日期序列（继承 Spark SQL）
```sql
...
FROM (SELECT explode(sequence(DATE '2024-01-01', DATE '2024-01-10', INTERVAL 1 DAY)) AS d)
...
注意：Databricks SQL 的日期序列生成方式见上述代码
