# MariaDB: 间隙检测与岛屿问题 (Gap Detection & Islands)

> 参考资料:
> - [MariaDB Documentation - Window Functions](https://mariadb.com/kb/en/window-functions/)
> - [MariaDB Documentation - Recursive CTEs](https://mariadb.com/kb/en/recursive-common-table-expressions-overview/)
...
## 1. 使用 LAG/LEAD 窗口函数查找数值间隙（MariaDB 10.2+）
...
MariaDB 10.3+ 的 seq_1_to_N 序列引擎
```sql
SELECT s.seq AS missing_id
FROM seq_1_to_100 s
WHERE s.seq BETWEEN (SELECT MIN(id) FROM orders) AND (SELECT MAX(id) FROM orders)
  AND s.seq NOT IN (SELECT id FROM orders);
```
...
注意：MariaDB 默认递归 CTE 深度 1000（max_recursive_iterations）
