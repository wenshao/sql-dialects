-- MaxCompute (ODPS): 间隙检测与岛屿问题 (Gap Detection & Islands)
--
-- 参考资料:
--   [1] MaxCompute Documentation - Window Functions
--       https://www.alibabacloud.com/help/en/maxcompute/user-guide/window-functions
--   [2] MaxCompute Documentation - SQL Reference
...
CREATE TABLE orders (id BIGINT, info STRING);
INSERT INTO orders VALUES (1,'a'),(2,'b'),(3,'c'),(5,'e'),(6,'f'),
    (10,'j'),(11,'k'),(12,'l'),(15,'o');

CREATE TABLE daily_sales (sale_date STRING, amount DECIMAL(10,2));
...
FROM (SELECT id, LEAD(id, 1) OVER (ORDER BY id) AS next_id FROM orders) t
WHERE next_id - id > 1;
...
SELECT sale_date, next_date,
       DATEDIFF(TO_DATE(next_date, 'yyyy-MM-dd'), TO_DATE(sale_date, 'yyyy-MM-dd'), 'dd') - 1 AS missing_days
FROM (
    SELECT sale_date, LEAD(sale_date, 1) OVER (ORDER BY sale_date) AS next_date
    FROM daily_sales
) t WHERE DATEDIFF(TO_DATE(next_date, 'yyyy-MM-dd'), TO_DATE(sale_date, 'yyyy-MM-dd'), 'dd') > 1;
...
-- 注意：MaxCompute 支持窗口函数
-- 注意：MaxCompute 不支持递归 CTE
-- 注意：MaxCompute 日期通常用 STRING 类型存储
-- 注意：使用 posexplode + split(space(n)) 生成序列
