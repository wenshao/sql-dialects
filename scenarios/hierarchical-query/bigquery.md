# BigQuery: 层次查询与树形结构 (Hierarchical Query & Tree Traversal)

> 参考资料:
> - [1] BigQuery Documentation - Recursive CTEs
>   https://cloud.google.com/bigquery/docs/reference/standard-sql/query-syntax#recursive_cte
> - [2] BigQuery Documentation - Hierarchical Data
...
CREATE TABLE employees (
    ...
    parent_id INT64, FOREIGN KEY (parent_id) REFERENCES employees(id),
    dept      STRING
);
...
WITH RECURSIVE org_tree AS (
    SELECT id, name, parent_id, 0 AS level,
           CAST(name AS STRING) AS sort_path
...
注意：BigQuery 从 2021 年开始支持递归 CTE
注意：BigQuery 默认递归深度 500
注意：BigQuery 不支持 CONNECT BY
注意：BigQuery 使用 CONCAT 连接字符串
