-- ClickHouse: 层次查询与树形结构 (Hierarchical Query & Tree Traversal)
--
-- 参考资料:
--   [1] ClickHouse Documentation - WITH RECURSIVE
--       https://clickhouse.com/docs/en/sql-reference/statements/select/with
--   [2] ClickHouse Documentation - Recursive CTEs
--       https://clickhouse.com/docs/en/sql-reference/statements/select/with#recursive-cte
...
CREATE TABLE employees (
    ...
    parent_id UInt32, FOREIGN KEY (parent_id) REFERENCES employees(id),
    dept      String
);
-- ClickHouse 需要 MergeTree 引擎
...
-- 注意：ClickHouse 22.9+ 支持递归 CTE（实验性）
-- 注意：需要设置 allow_experimental_analyzer = 1
-- 注意：ClickHouse 不支持 CONNECT BY
-- 注意：递归 CTE 在 ClickHouse 中性能有限，建议使用物化路径
