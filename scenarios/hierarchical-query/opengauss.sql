-- openGauss: 层次查询与树形结构 (Hierarchical Query & Tree Traversal)
--
-- 参考资料:
--   [1] openGauss Documentation - Recursive CTEs
--       https://docs.opengauss.org/en/docs/latest/docs/SQLReference/
--   [2] openGauss Documentation - CONNECT BY
--       https://docs.opengauss.org/en/docs/latest/docs/SQLReference/
...
-- 注意：openGauss 基于 PostgreSQL，支持递归 CTE
-- 注意：openGauss 也支持 CONNECT BY（Oracle 兼容模式）
-- 注意：openGauss 支持 START WITH ... CONNECT BY 语法
-- 注意：openGauss 使用 || 连接字符串
