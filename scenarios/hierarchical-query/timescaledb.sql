-- TimescaleDB: 层次查询与树形结构 (Hierarchical Query & Tree Traversal)
--
-- 参考资料:
--   [1] TimescaleDB Documentation - SQL Reference
--       https://docs.timescale.com/api/latest/
--   [2] PostgreSQL Documentation - WITH RECURSIVE
...
-- 注意：TimescaleDB 是 PostgreSQL 扩展，完整支持递归 CTE
-- 注意：TimescaleDB 不支持 CONNECT BY
-- 注意：递归 CTE 语法与 PostgreSQL 完全相同
-- 注意：TimescaleDB 的时序优化不影响递归查询
