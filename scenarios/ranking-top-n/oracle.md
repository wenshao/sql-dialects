...
## LATERAL / CROSS APPLY（12c+，分组 Top-N 的优雅方案）
...
## KEEP (DENSE_RANK): Oracle 独有的组内取值聚合
...
性能排序:
### CROSS APPLY + FETCH FIRST: 最优（每组只计算 N 行）

### ROW_NUMBER + WHERE rn <= N: 优化器可优化为 Top-N 排序

### KEEP (DENSE_RANK): 纯聚合，只能取 Top-1 但最快

### ROWNUM: Pre-12c 的最优选择
