...
> 参考资料:
> - [CockroachDB Documentation - INSERT ON CONFLICT (UPSERT)](https://www.cockroachlabs.com/docs/stable/insert)
> - [CockroachDB Documentation - UPDATE](https://www.cockroachlabs.com/docs/stable/update)
> - [CockroachDB Documentation - Transactions](https://www.cockroachlabs.com/docs/stable/transactions)
...
## CockroachDB 注意事项与最佳实践


## CockroachDB 高度兼容 PostgreSQL 语法，包括 ON CONFLICT

## CockroachDB 不支持可写 CTE（WITH ... DML ... RETURNING）

   因此 SCD Type 2 必须分步执行，不能使用 PostgreSQL 的单语句方案
## 分布式事务使用 SERIAL 隔离级别（默认），保证强一致性

## SERIAL 类型在 CockroachDB 中生成有序唯一 ID（非自增）

## 对于高频写入场景，建议使用 UPSERT 代替 INSERT ON CONFLICT

## CockroachDB 的 RANGE 分片自动管理，无需手动指定分布键

## 大规模 ETL 操作建议使用批量导入（IMPORT）而非逐行 INSERT

## CockroachDB 支持 CHANGEFEED（CDC），可监听维度表变更
