...
--
-- 参考资料:
--   [1] 达梦数据库 SQL 参考手册 - MERGE INTO
--       https://eco.dameng.com/document/dm/zh-cn/sql-dev/
--   [2] 达梦数据库 SQL 参考手册 - 数据类型
--       https://eco.dameng.com/document/dm/zh-cn/sql-dev/
--   [3] 达梦数据库 兼容 Oracle 模式 MERGE 语法
--       https://eco.dameng.com/document/dm/zh-cn/pm/sql-reference/
...
-- ============================================================
-- 3. SCD Type 1: MERGE INTO（兼容 Oracle 语法）
-- ============================================================
...
-- 1. 达梦 MERGE 语法高度兼容 Oracle，适合从 Oracle 迁移的项目
-- 2. is_current 使用 CHAR(1) 而非 BOOLEAN，这是 Oracle 风格惯例
-- 3. SYSDATE 是达梦的当前时间函数（兼容 Oracle），也可用 CURRENT_DATE
-- 4. 达梦 IDENTITY 语法: INT IDENTITY(1,1) 类似 SQL Server
-- 5. 达梦支持 SQL:2003 MERGE 标准，可在 WHEN MATCHED 中添加条件过滤
-- 6. 在达梦集群部署中，建议使用哈希分区优化大维度表的查询
-- 7. 达梦支持事务内 DDL 操作，可在同一事务中修改表结构和数据
