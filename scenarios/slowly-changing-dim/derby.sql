...
--       https://db.apache.org/derby/docs/10.17/ref/
--   [2] Apache Derby MERGE Statement (10.11+)
--       https://db.apache.org/derby/docs/10.17/ref/rrefsqljmerge.html
...
-- ============================================================
-- 3. SCD Type 1: UPDATE + INSERT（传统方式）
-- ============================================================
...
-- ============================================================
-- 5. SCD Type 2: 使用 MERGE（Derby 10.11+）
-- ============================================================
...
-- 1. Derby 10.11+ 支持 MERGE，可用于 SCD Type 1
-- 2. Derby 不支持可写 CTE（WITH ... UPDATE ... INSERT），SCD Type 2 需分步执行
-- 3. Derby 不支持 UPSERT / ON CONFLICT / ON DUPLICATE KEY
-- 4. GENERATED ALWAYS AS IDENTITY 提供自增主键（Derby 10.7+）
-- 5. SCD Type 2 的两步操作不在同一事务中，建议手动 BEGIN / COMMIT 包裹
