...
> 参考资料:
> - [OceanBase Documentation - INSERT ON DUPLICATE KEY UPDATE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000157744)
> - [OceanBase Documentation - UPDATE](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000157749)
> - [OceanBase Documentation - MySQL Compatibility](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000218464)
...
## OceanBase MySQL 模式高度兼容 MySQL 语法，可直接复用 MySQL ETL 脚本

## OceanBase 也支持 Oracle 模式（兼容 Oracle PL/SQL、MERGE 等）

## 分布式架构下，建议使用 HASH 分区优化大维度表

   ALTER TABLE dim_customer PARTITION BY HASH(customer_id) PARTITIONS 16;
## ON DUPLICATE KEY UPDATE 性能优于 REPLACE INTO（避免 DELETE + INSERT 开销）

## OceanBase 支持多租户隔离，建议为 ETL 工作负载创建独立租户

## 使用 OceanBase CDC (LogProxy) 可实现增量抽取，辅助 SCD Type 2

## is_current 使用 TINYINT(1/0) 而非 BOOLEAN，符合 MySQL 兼容惯例
