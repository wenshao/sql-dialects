...
--
-- 参考资料:
--   [1] TiDB Documentation - INSERT ON DUPLICATE KEY UPDATE
--       https://docs.pingcap.com/tidb/stable/sql-statement-insert
--   [2] TiDB Documentation - REPLACE INTO
--       https://docs.pingcap.com/tidb/stable/sql-statement-replace
--   [3] TiDB Documentation - UPDATE
...
-- 1. TiDB 高度兼容 MySQL 5.7 语法，包括 ON DUPLICATE KEY UPDATE
-- 2. TiDB 是分布式 HTAP 数据库，OLTP 和 OLAP 混合负载
--    对于 SCD 查询，TiFlash 列存引擎可加速分析:
--    ALTER TABLE dim_customer SET TIFLASH REPLICA 1;
-- 3. UPDATE 在 TiDB 中可能产生大量写放大（MVCC 机制）
--    建议使用 INSERT ON DUPLICATE KEY UPDATE 代替分步 UPDATE + INSERT
-- 4. TiDB 的 AUTO_INCREMENT 在分布式环境下不保证严格递增
--    建议使用 AUTO_ID_CACHE 或业务层生成代理键
-- 5. 大规模数据导入推荐使用 TiDB Lightning 或 LOAD DATA
-- 6. TiDB CDC (TiCDC) 可捕获维度表变更，辅助 SCD Type 2 增量处理
-- 7. TiDB 支持 Placement Rules，可将维度表放置在特定节点:
