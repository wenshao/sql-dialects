...
--   [2] TiDB Documentation - SQL Functions
--       https://docs.pingcap.com/tidb/stable/functions-and-operators-overview

-- 注意：窗口函数需要 TiDB 3.0+
-- TiDB 兼容 MySQL 8.0 窗口函数语法

-- 假设表结构:
--   daily_sales(sale_date DATE, product_id INT, region VARCHAR(50),
--               amount DECIMAL(10,2), quantity INT)
--   user_events(user_id INT, event_time DATETIME, event_type VARCHAR(50), page VARCHAR(255))
--   employee_salaries(emp_id INT, department VARCHAR(50), salary DECIMAL(10,2), hire_date DATE)
...
-- TiDB 目前不支持 PERCENTILE_CONT / PERCENTILE_DISC
-- 使用 NTILE 模拟
WITH ranked AS (
...
-- 分布式执行，大表窗口查询会触发 Coprocessor 计算
