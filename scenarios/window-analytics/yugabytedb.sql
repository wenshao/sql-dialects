...
--
-- 参考资料:
--   [1] YugabyteDB Documentation - Window Functions (YSQL)
--       https://docs.yugabyte.com/latest/api/ysql/exprs/window_functions/
--   [2] PostgreSQL Documentation (YSQL 兼容 PostgreSQL)

-- YSQL 模式兼容 PostgreSQL，窗口函数语法完全相同
-- 假设表结构同 PostgreSQL
...
-- YugabyteDB YSQL 完全兼容 PostgreSQL 窗口函数
-- 分布式环境下窗口函数可能触发跨节点数据传输
-- 将 PARTITION BY 列与表的分区键对齐可优化性能
