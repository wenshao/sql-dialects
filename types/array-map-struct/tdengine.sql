...
--
-- 参考资料:
--   [1] TDengine Documentation - Data Types
--       https://docs.tdengine.com/taos-sql/data-type/
--   [2] TDengine Documentation - JSON Type
--       https://docs.tdengine.com/taos-sql/json/
...
-- 1. 不支持原生 ARRAY / MAP / STRUCT 列类型
-- 2. JSON 类型只能用于超级表的标签（Tag），不能用于普通列
-- 3. 普通列只支持标量类型
-- 4. 时序数据通常不需要复杂类型（每个度量点是标量值）
-- 5. 使用超级表 + 标签的架构来组织元数据
