...
-- 1. TDSQL 没有原生 ARRAY/MAP/STRUCT，使用 JSON 类型替代
-- 2. JSON 数组替代 ARRAY: 使用 JSON_ARRAY / JSON_ARRAY_APPEND
-- 3. JSON 对象替代 MAP/STRUCT: 使用 JSON_OBJECT / JSON_SET
-- 4. JSON_TABLE 是最强大的工具: JSON 数组→关系表的桥梁
-- 5. JSON 聚合函数 (JSON_ARRAYAGG/JSON_OBJECTAGG) 实现行→JSON 转换
-- 6. JSON 列不能作为 shardkey
-- 7. 高频 JSON 字段建议提取为虚拟列并建索引
-- 8. 参见 mysql.sql 获取完整的 MySQL JSON 函数列表
