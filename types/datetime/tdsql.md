...
## 所有分片的系统时区必须统一设置（推荐 UTC）

## TIMESTAMP 有 2038 年问题，长期存储推荐 DATETIME

## 记录创建/更新时间可用 TIMESTAMP 的自动更新特性

## 时间类型不建议作为 shardkey（写入热点问题）

## 推荐使用 NOW() 而非 SYSDATE()（同一事务时间一致性）

## 高精度时间（fsp=3 或 fsp=6）便于分布式事务排序

## 跨分片时间范围查询需注意性能，考虑按时间维度分表
