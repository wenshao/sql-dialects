...
--
-- 参考资料:
--   [1] DamengDB SQL Reference - JSON Functions
...
--   [3] SQL/JSON Standard (ISO/IEC 9075-2:2016)
...
-- 1. 达梦使用 SQL/JSON 标准函数（JSON_VALUE、JSON_QUERY、JSON_TABLE）
-- 2. 没有原生 JSON 列类型，使用 CLOB 或 VARCHAR 存储
-- 3. 不支持 MySQL 风格的 -> 和 ->> 运算符
-- 4. JSON_TABLE 功能强大，推荐用于 JSON 数据的关系化查询
-- 5. JSON 路径表达式对齐 Oracle 的 SQL/JSON 标准
-- 6. 高频查询的 JSON 字段建议创建函数索引
-- 7. Oracle 迁移场景下 JSON 函数基本兼容
-- 8. 大 JSON 文档使用 CLOB，小 JSON 使用 VARCHAR（性能更优）
