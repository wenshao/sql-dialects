...
## Materialize 与 PostgreSQL JSONB 的差异
...
## 仅支持 JSONB 类型（不支持 JSON 文本类型）

## 兼容 PostgreSQL 的 JSONB 操作符和函数

## 使用物化视图替代索引来加速 JSON 查询

## 不支持 JSONB 原地修改操作（流式模式，数据不可变）

## Kafka SOURCE 支持 FORMAT JSON 自动解析

## 建议在物化视图中预提取高频 JSON 字段

## JSON 文档大小影响摄入吞吐量，建议控制在合理范围

## jsonb_typeof() 可检查 JSON 值类型（number, string, array, object 等）
