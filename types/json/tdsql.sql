...
--   [3] MySQL 8.0 Reference Manual - JSON Data Type
...
-- 1. JSON 类型与 MySQL 完全兼容，使用 $. 路径语法
-- 2. JSON 列不能作为 shardkey，需提取为独立列
-- 3. JSON 函数在各分片独立执行，跨分片聚合由代理层合并
-- 4. 高频查询的 JSON 字段建议使用虚拟列 + 索引加速
-- 5. JSON 文档建议控制在合理大小，避免大 JSON 的分布式传输开销
-- 6. JSON_SET / JSON_INSERT / JSON_REPLACE 语义不同，注意区分
-- 7. JSON_TABLE 功能强大，适合 JSON 数据的关系化查询
-- 8. 所有分片的 JSON 函数行为一致，无需担心分布式差异
