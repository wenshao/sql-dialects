...
--   [4] MySQL 8.0 Reference Manual - Type Conversion in Expression Evaluation
...
-- ============================================================
-- 2. BIGINT vs INT: 21 亿上限问题（对引擎开发者）
-- ============================================================

-- 2.1 INT 的上限: 2,147,483,647 (约 21.47 亿)
-- 看似很大，但在以下场景很快耗尽:
--   - 高频写入: 每秒 1000 条 INSERT → 约 24.8 天耗尽 UNSIGNED INT
--   - 分布式 ID: 多节点预分配 ID 段时，有效范围进一步缩小
--   - 日志/事件表: IoT 设备每秒多条记录，INT 不够用
--
-- 2.2 BIGINT 的范围: 9,223,372,036,854,775,807 (约 922 京)
...
-- ============================================================
-- 3. DECIMAL 精确计算 vs FLOAT/DOUBLE IEEE 754 精度损失
-- ============================================================
...
-- ============================================================
-- 4. UNSIGNED 的废弃趋势和设计反思
-- ============================================================

-- 4.1 UNSIGNED 整数: MySQL 独有特性
-- INT UNSIGNED: 0 ~ 4,294,967,295（正数范围翻倍）
-- 看似有用，但带来了严重的语义问题:

-- 4.2 UNSIGNED 的陷阱: 减法溢出
