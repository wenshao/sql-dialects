...
--   [3] MySQL 8.0 Reference Manual - Numeric Data Types
...
-- 1. 数值类型与 MySQL 完全兼容，所有 MySQL 数值函数均可用
-- 2. shardkey 列推荐使用 INT 或 BIGINT，hash 分布最均匀
-- 3. AUTO_INCREMENT 保证全局唯一但不连续，不要依赖连续性
-- 4. 金融计算必须使用 DECIMAL，绝对不能用 FLOAT/DOUBLE
-- 5. BIT 类型不能作为 shardkey
-- 6. UNSIGNED 类型可用但建议避免，直接用更大类型替代
-- 7. 跨分片聚合在协调节点汇总，大结果集注意内存消耗
-- 8. 浮点数跨分片 SUM 可能因累积顺序不同产生微小差异
