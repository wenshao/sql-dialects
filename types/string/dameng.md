...
## VARCHAR 和 VARCHAR2 功能相同，Oracle 迁移用 VARCHAR2，新项目均可

## CHAR/VARCHAR 的 n 是字节数（不是字符数！），设计表时需考虑编码

## NVARCHAR/NVARCHAR2 的 n 是字符数，多语言场景推荐使用

## CLOB 用于大文本存储，提供 Oracle 兼容的 DBMS_LOB 操作

## 字符集在数据库初始化时设置，不可更改，建议 UTF-8

## 大小写敏感性在初始化时配置，Oracle 迁移建议设为不敏感

## PAGE 大小影响 VARCHAR 最大长度，大字段场景建议 PAGE=32K

## 支持 RAW 类型存储二进制数据，Oracle 兼容迁移可直接使用
