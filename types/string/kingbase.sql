...
--   [3] PostgreSQL Documentation - String Types
...
CREATE TABLE string_examples (
    -- 定长: 右侧补空格到 n 个字符，最大约 1GB
    country_code  CHAR(2)           NOT NULL,    -- 'US', 'CN', 'JP'
    -- 变长: 最大约 1GB（受 TOAST 溢出机制支持）
...
-- 1. 字符串类型与 PostgreSQL 完全兼容，可无缝迁移
-- 2. Oracle 兼容模式下支持 VARCHAR2、CLOB、NVARCHAR2 等类型
-- 3. TEXT 类型无长度限制，日常使用推荐 TEXT
-- 4. 字符集在 CREATE DATABASE 时确定，之后不可更改
-- 5. 支持多种字符集（UTF-8、GBK、GB18030 等），推荐 UTF-8
-- 6. 中文全文搜索需安装 zhparser 扩展
-- 7. 不支持 MySQL 的 ENUM 和 SET 类型，使用 CHECK 约束替代
-- 8. LENGTHB 函数是 Oracle 兼容扩展，返回字节数（区别于 LENGTH 返回字符数）
