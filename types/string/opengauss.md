...
## 不支持 MySQL 的 ENUM 和 SET 类型（可使用 CHECK 约束替代）

## TEXT 类型无长度限制但内部使用 TOAST 溢出机制，超大文本性能可能下降

## NVARCHAR2 是 openGauss 扩展，迁移到原生 PostgreSQL 需改为 VARCHAR/TEXT

## CLOB 与 TEXT 内部机制相同，Oracle 迁移场景用 CLOB，新项目用 TEXT

## 字符集在 CREATE DATABASE 时确定，之后不可更改

## 建议使用 UTF-8 编码以获得完整的 Unicode 支持

## 排序规则可按列/表达式设置，注意 JOIN 时的 COLLATION 一致性
