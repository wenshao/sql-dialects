...
## 字符串类型与 MySQL 完全兼容，所有 MySQL 字符串函数均可用

## shardkey 列建议使用 VARCHAR 或 INT 类型，避免 TEXT/BLOB

## 所有分片的字符集和 COLLATION 必须统一配置

## TEXT/BLOB 列不能作为 shardkey，需通过其他列路由

## 默认字符集 utf8mb4，永远不要使用 utf8（3 字节版本）

## VARCHAR(n) 的 n 应根据实际需求设定，过大的 n 影响分布式性能

## 跨分片字符串 JOIN 注意 COLLATION 一致性

## ENUM 值的 DDL 变更在分布式环境下需评估影响范围
